...
perturbations. The dashed line is too large of a bandwidth (\(h\) =
2.1), so it over-smooths the interesting components of the distribution.
The heavy line (\(h\) = 0.6) seems to be a fairly respectable bandwidth;
it does not over-smooth the data, and it also highlights some of the
...
in the data. This calculation requires \(n (n-1)\) computations to find
the final estimate. To get around such a computationally intensive task,
techniques such as linear binning and Fast Fourier Transforms are used
(both are implemented in \texttt{R}'s \texttt{density} function).
...
choice would be the Integrated Squared Error (ISE):
...
performance on average over many data sets.
\end{quote}
...
heavy dependence on the observed data often leads to high variation in
...
Terrell (1990) utilized a different approach in determining an optimal
...
selection method does a fairly good job of estimating the density.
\(h_{SROT}\) relies on a normal approximation, so we would expect this
bandwidth to perform well on this vector. In fact, it is the most
aggressive bandwidth estimator, resulting in the smallest \(h\) value of
the four. A \emph{best bandwidth} is not apparent from these results.
...
for a discerning data analyst. We also see that \(h_{MS}\) provides a
much smoother estimate than those proposed by Silverman and Sheather and
Jones; it seems that it is too conservative. The best bandwidth would
take a value somewhere in the neighborhood of \(h_{SJ}\) and
\(h_{SROT}\).
...
analyzing their data. One bandwidth should have the tendency to
over-smooth (e.g. \(h_{SROT}\) or \(h_{MS}\)), and another bandwidth
...
top 250 beers as rated on BeerAdvocate.com) provided below, compute and
