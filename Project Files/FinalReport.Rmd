...
The kernel function is generally chosen to be a unimodal probability density function that is symmetric around zero. Popular choices include the Gaussian kernel ($K(x) = \phi(x)$), the Epanechnikov kernel ($K(x) = \frac{3}{4}(1-x^2)*\mathbb{I}(\left|x \right| <1)$), and the triangular kernel ($K(x) = (1 - \left|x\right|)*\mathbb{I}(\left|x\right| < 1)$). Provided the kernel function $K(x)$ satisfies the aforementioned conditions, the choice of kernel will not be as important the selection of a bandwidth.
...
Along with the computation of $\widehat{f_h}(x)$, it is also important to assess the quality of $\hat{f}$ as an estimator of $f$. One choice would be the Integrated Squared Error (ISE):
...
where $\sigma_K^2$ is the variance of $K$ ($\int z^2 K(z) dz$). So that $\left(bias(\widehat{f_h}(x))\right)^2 = \frac{1}{4} h^4\sigma_K^4 (f^{''})^2 + o(h^4)$, and integrating $bias^2$ results in:

$$\int bias\left(\hat{f}(x)\right)^2 dx = \frac{1}{4} h^4 \sigma_K^4 R(f^{''}) + o(h^4),$$

where $R$ is a measure of the roughness of a function g:

$$R(g) = \int_{-\infty}^{\infty}g^2\left(z\right) dz.$$
...
$$
ISE(h) = \int \left(\hat{f}_h - f\right)^2 = \int\hat{f}_h^2 - 2\int \hat{f}_hf + \int f^2.
$$
...
is the leave-one-out kernel density estimator at $x_i$ using all of the data _except_ $x_i$.
...
For this report, we have focused on the Gaussian kernel. $UCV(h)$ has a straightforward expression when a Gaussian kernel is used (equation 10.23, @Givens2013[p. 333]), which makes the computation of $h_{LSCV}$ rather tidy.

The unbiasedness of the objective function is a nice feature, but it comes at the cost of imbuing $h_{LSCV}$ with excessive variance [@Jones1992, p. 6]. The objective function $UCV$ tends to depend too heavily on the sample in hand, since it is still random (unlike $MISE$, which is averaged over _all_ samples). The heavy dependence on the observed data often leads to high variation in $\hat{f}_h$. This excessive variation is displayed in the _Examples_ section of this report.
...
Here we see the excessive variation caused by using $h_{LSCV}$. The density estimate provided by $h_{LSCV}$ is far too wiggly to be useful for a discerning data analyst. We also see that $h_{MS}$ provides a much smoother estimate than those proposed by Silverman and Sheather and Jones; it seems that it is too conservative. The best bandwidth would take a value somewhere in the neighborhood of $h_{SJ}$ and $h_{SROT}$.

\newpage
\subsection{Example 4: Old Faithful Waiting Time}

Using data from @Bowman1990, contained in the __`MASS`__ package [@Venables], we calculate the kernel density estimate for the waiting time for eruptions from August 1 to August 15, 1985 at the Old Faithful geyser in Yellowstone National Park, Wyoming.
...
A safe option for a data analyst is to use at least two bandwidths when analyzing their data. One bandwidth should have the tendency to over-smooth (e.g. $h_{SROT}$ or $h_{MS}$), and another bandwidth should produce estimates that are more wiggly (e.g. $h_{SJ}$, __but not $h_{LSCV}$__). If visualizing is the goal (rather than simulation), then including a histogram will provide even more clarity when making density plots (note that making a histogram also requires selecting a bandwidth, though).
...
Using the `density` function and the beer data (`abv.complete`, which are the alcohol by volume measures for the top 250 beers as rated on BeerAdvocate.com) provided below, compute and plot a 95% pointwise bootstrapped percentile interval. Additionally, use the approximate variance function $\tilde{V}$ given below and found in @Jann2007[p. 2] to compute and plot a 95% pointwise confidence interval. Create two plots for each method using `bw.SJ` and `bw.nrd0`. 
...
ggplot(app.abv.SJ, aes(x = grid))+
    ...
    xlab('ABV')+
    ylab('Density')+
    ggtitle('ABV Density and 95% Approximate Pointwise CI',
            subtitle = 'Sheather - Jones Bandwidth')
...
To visit the Shiny app, __[click this link](http://www.statlab.wisc.edu/shiny/Bandwidths_For_UnivariateKDE/)__.
